...
만약 휴대폰 쇼핑몰에서 Galaxy Note라는 클래스를 만든다고 하면, 이름과 색상, 가격은 정적 특성인 attribute가 될것이고 동적 특성에는 개개인에 맞게 할부원금을 계산하는 것이 있겠습니다.

다른 예로, 만약 삼성에서 제조관리 시스템을 제작하기위해 똑같이 Galaxy Note라는 클래스를 만든다면 시리얼넘버,재질,조립상태등의 정적 특성으로 분류되어 질것이고, 동적 특성에는 불량을 확인하는 행위가 있겠습니다.
